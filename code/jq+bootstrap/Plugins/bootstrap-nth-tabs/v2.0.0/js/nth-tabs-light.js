...
(function ($) {
    ...
    $.fn.nthTabs = function (options) {
        ...
        var template =
            ...
            '</div>' +
            '</div>';
        //'<div class="tab-content"></div>'; // fix by light

        // 启用插件
        var run = function () {
            // fix by light
            if (settings.tabContentId == '') {
                settings.tabContentId = '#main-nth-tabs-content';
                template += '<div class="tab-content" id="main-nth-tabs-content"></div>';
            }
        ...
        var methods = {
            ...
            addTab: function (options) {
                // 获取 content-wrapper 高度
                var _contentWrapper = $(settings.tabContentId).parent();
                //var _tabContentHeight = parseInt(_contentWrapper.css('min-height')) - 40;
                //var _tabContentHeight = _contentWrapper.height() - 40;
                var _tabContentHeight = parseInt(_contentWrapper.css('min-height'));
                if (_tabContentHeight <= 120)
                    _tabContentHeight = _contentWrapper.height() - 40;
                else
                    _tabContentHeight -= 40;
                ...
                //tab-content
                var tabContent = [];
                tabContent.push('<div class="tab-pane ' + (fadeIn ? 'animation-fade' : '') + '" id="' + options.id + '" style="height:' + _tabContentHeight + 'px">');
                //tabContent.push('<div class="tab-pane ' + (fadeIn ? 'animation-fade' : '') + '" id="' + options.id + '" style="height:calc(100vh + 180px);">');

                var _iframeId = ''; // fix by light
                var _random = Math.random();
                if (url.length > 0) {
                    _iframeId = 'iframe-' + frameName;
                    tabContent.push('<iframe src="' + options.url + '&_r='+_random+'" frameborder="0" id="' + _iframeId + '" name="' + _iframeId + '" class="nth-tabs-frame" scrolling="no" height="100%"></iframe>');
                    frameName++;
                } else {
                ...
                $(settings.tabContentId).append(tabContent.join(''));
            ...
            delOtherTab: function () {
                ...
                $(settings.tabContentId).find('div.tab-pane').not('[class$="active"]').remove();
            ...
            delAllTab: function () {
                ...
                $(settings.tabContentId).find('div').remove();
                return this;
            },
            ...
            setActTab: function (tabId) {
                ...
                $(settings.tabContentId).find('.active').removeClass('active')
                $(settings.tabContentId).find(tabId).addClass('active'); // fix by light
                return this;
            },
            ...
            isExistsTab: function (tabId) {
                ...
                return $(settings.tabContentId).find(tabId).length > 0; // fix by light
            }
        };
